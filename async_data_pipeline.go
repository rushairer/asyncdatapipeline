...
type AsyncDataPipelineConfig struct {
	...
	// CollectTimeout 指定数据采集的超时时间
	// 当采集函数超过此时间仍未返回时，将返回错误
	...
	go func() {
		...
		for {
			...
			default:
				...
				case <-ctx.Done():
					return
				case <-time.After(p.config.CollectTimeout):
					p.errorMu.Lock()
					p.errorList = append(p.errorList, &CollectError{
						Err: fmt.Errorf(
							"collect timeout: %v",
							p.config.CollectTimeout,
						),
					})
					p.errorMu.Unlock()
					return
